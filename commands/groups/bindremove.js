module.exports = {
    ...
    Run: async(Service, message, args)=>{
        if (!args) return Service.Utils.ReturnMessage(message, `RobloxNUSA.Ver :: Error`,`Requires 1 parameters.`, [{name:"Format", value:`${Service.Enviroment.Prefix}${Service.Command.Name} ${Service.Command.Usage}`}]);
        if (args.length != 1 && message.content.indexOf('"')  == -1) return Service.Utils.ReturnMessage( message, `RobloxNUSA.Ver :: Error`,`Requires 1 parameters. ${args.length} were supplied.`, [{name:"Format", value:`${Service.Enviroment.Prefix}${Service.Command.Name} ${Service.Command.Usage}`}]);
        if (!parseInt(args[0]) && message.content.indexOf('"')  == -1) return Service.Utils.ReturnMessage(message, `RobloxNUSA.Ver :: Error`,`Invalid Role format.`, [{name:"Example", value:`${Service.Enviroment.Prefix}unbind "American Citizen"`}]);
