    ...
    Run: async(Service, message, args)=>{
        if (args.length === 1){
            if (parseInt(args[0])){
                ...
                return Service.Utils.ReturnMessage(message, `RobloxNUSA.Ver :: Error`,`Please supply a numericl arguement.`, [{name:"Format", value:`${Service.Enviroment.Prefix}${Service.Command.Name} ${Service.Command.Usage}`}]);
            }
        } else {
            return Service.Utils.ReturnMessage(message, `RobloxNUSA.Ver :: Error`,`Please supply only one arguement.`, [{name:"Format", value:`${Service.Enviroment.Prefix}${Service.Command.Name} ${Service.Command.Usage}`}]);
        }
    }
}